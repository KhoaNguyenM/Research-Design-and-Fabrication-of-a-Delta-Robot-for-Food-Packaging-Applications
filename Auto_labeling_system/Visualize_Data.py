...
class YoloImageViewer:
    def __init__(self, root):
        ...
        self.class_colors = {}  # Ánh xạ class ID -> màu RGB
        ...
        # Nút xóa ảnh
        self.btn_delete = tk.Button(self.nav_frame, text="Xóa ảnh", command=self.delete_current_image, 
    ...
    def get_class_color(self, class_id):
        if class_id not in self.class_colors:
            np.random.seed(class_id)  # để màu sắc ổn định mỗi lần chạy
    ...
    def load_image_files(self):
        ...
        if self.image_files:
            ...
            self.btn_delete.config(state=tk.NORMAL if self.image_files else tk.DISABLED)
    ...
    def delete_current_image(self):
        ...
        # Lấy tên file ảnh và label hiện tại
        ...
        if result:
            try:
                ...
                # Xóa file label nếu tồn tại
                if os.path.exists(label_path):
                    os.remove(label_path)
                
                # Cập nhật danh sách file
                self.image_files.remove(image_filename)
                ...
                if not self.image_files:
                    ...
                    self.lbl_filename.config(text="Không có ảnh để hiển thị")
                    self.lbl_image_info.config(text="0/0")
                    self.btn_prev.config(state=tk.DISABLED)
                    ...
                    self.btn_next.config(state=tk.NORMAL if self.current_index < len(self.image_files) - 1 else tk.DISABLED)
                    
                    # Hiển thị ảnh tiếp theo
                    self.show_current_image()
                
                messagebox.showinfo("Thành công", "Đã xóa ảnh và label thành công!")
